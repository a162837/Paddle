...
#             min_data = np.random.random([9, 4]).astype('float32')
#             max_data = np.random.random([9, 9, 4]).astype('float32')
#             images = paddle.static.data(
#                 name='image', shape=data_shape, dtype='float32'
#             )
#             min = paddle.static.data(name='min', shape=[9, 4], dtype='float32')
#             max = paddle.static.data(name='max', shape=[9, 9, 4], dtype='float32')
...
#             out_2 = self._executed_api(images, min=min, max=0.9)
#             out_5 = self._executed_api(images, min=min)
#             out_6 = self._executed_api(images, max=max)
#             out_7 = self._executed_api(images, min=0.1, max=max)
#             out_8 = self._executed_api(images)
#             res1, res2, res5, res6, res7, res8 = exe.run(
#                 train_prog,
#                 feed={
#                     "image": data,
#                     "min": min_data,
...
#             np.testing.assert_allclose(res1, data.clip(min_data, max_data))
#             np.testing.assert_allclose(res2, data.clip(min_data, 0.9))
#             np.testing.assert_allclose(res5, data.clip(min=min_data))
#             np.testing.assert_allclose(res6, data.clip(max=max_data))
#             np.testing.assert_allclose(res7, data.clip(min=0.1, max=max))
...
#         v_min = paddle.to_tensor(min_data, dtype='float32')
#         v_max = paddle.to_tensor(max_data, dtype='float32')

#         out_1 = self._executed_api(images, min=v_min, max=0.8)
#         images = paddle.to_tensor(data, dtype='float32')
#         out_2 = self._executed_api(images, min=0.2, max=v_max)
#         images = paddle.to_tensor(data, dtype='float32')
#         out_3 = self._executed_api(images, min=v_min, max=v_max)

#         np.testing.assert_allclose(out_1.numpy(), data.clip(v_min, 0.8))
#         np.testing.assert_allclose(out_2.numpy(), data.clip(0.2, v_max))
#         np.testing.assert_allclose(out_3.numpy(), data.clip(v_min, v_max))
